class PhonenumbersController < ApplicationController
  ...
  def index
    @phonenumbers = Phonenumber.all.paginate(:per_page =>15, :page => params[:page])
                        .where("name ILIKE ?","%#{params[:name_search]}%").where("home ILIKE ?","%#{params[:home_search]}%").where("office ILIKE ?","%#{params[:office_search]}%").where("cell ILIKE ?","%#{params[:cell_search]}%")
  end
  ...
  def create
    ...
    respond_to do |format|
      if @phonenumber.save
        format.html { redirect_to @phonenumber, notice: 'Phone Number was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @phonenumber.update(phonenumber_params)
        format.html { redirect_to @phonenumber, notice: 'Phone Number was successfully updated.' }
  ...
  def destroy
    @phonenumber.destroy
    respond_to do |format|
      format.html { redirect_to phonenumbers_url, notice: 'Phone Number was successfully destroyed.' }
