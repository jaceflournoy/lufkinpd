class RollcallsController < ApplicationController
  ...
  # GET /rollcalls.json
  def index
    @rollcalls = Rollcall.all.paginate(:per_page =>15, :page => params[:page]).where("cast(radionum as varchar(20)) ILIKE ?","%#{params[:radionum_search]}%").where("title ILIKE ?","%#{params[:title_search]}%")
  end
